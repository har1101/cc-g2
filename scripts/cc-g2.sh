...
info()  { echo -e "${GREEN}[g2]${NC} $*"; }
warn()  { echo -e "${YELLOW}[g2]${NC} $*"; }
...
resolve_groq_api_key() {
  ...
  [ -n "$value" ] || value="$(read_env_file_var "$G2_PROJECT_DIR/.env.local" "VITE_GROQ_API_KEY" || true)"
  [ -n "$value" ] || value="$(read_env_file_var "$G2_PROJECT_DIR/.env" "GROQ_API_KEY" || true)"
  [ -n "$value" ] || value="$(read_env_file_var "$G2_PROJECT_DIR/.env" "VITE_GROQ_API_KEY" || true)"
  printf '%s' "$value"
}
...
ensure_infra() {
  local need_hub=false need_vite=false
...
if [ -z "${TMUX:-}" ] || [ "$FORCE_NEW_SESSION" = "1" ]; then
  ...
  if [ "$FORCE_NEW_SESSION" != "1" ] && tmux has-session -t "$TMUX_SESSION" 2>/dev/null; then
    ...
    exec tmux new-session -s "$TMUX_SESSION" -c "$WORK_DIR" \
      "${tmux_env[@]}" \
      "\"$0\" $*; exec \$SHELL"
  fi
fi
